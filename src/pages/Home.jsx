...
export default function Home() {
    ...
    const fetchJobs = async () => {
      ...
      try {
        const res = await fetch(
          "https://api.britishirishsocialworkagency.co.uk/api/jobs"
        );
        ...
        setError("Unable to load job openings at the moment.");
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    console.log("🔹 Filtering jobs for searchTerm:", searchTerm);
    const term = searchTerm.trim().toLowerCase();
    ...
    const filtered = jobs
      .filter((job) => {
        ...
        if (job.description) {
          ...
          location = match ? match[1].toLowerCase() : "";
        }
